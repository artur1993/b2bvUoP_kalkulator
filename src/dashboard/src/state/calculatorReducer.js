  ...
  switch (action.type) {
    case 'SET_B2B_FIELD': {
      ...
      const b2bData = name.startsWith('companyBenefits.')
        ...
        : { ...state.b2bData, [name]: inputType === 'checkbox' ? checked : value };
      return { ...state, b2bData };
    }
    case 'SET_UOP_FIELD': {
      const { name, value, inputType, checked } = action.payload;
      return { ...state, uopData: updateUopField(state.uopData, name, value, inputType, checked) };
    }
    case 'SET_AGE': {
    ...
    case 'HYDRATE_FROM_URL':
      return {
        ...state,
        calculationMode: action.payload.calculationMode ?? state.calculationMode,
