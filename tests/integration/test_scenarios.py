...
class ScenariosTestCase(unittest.TestCase):
    ...
    def test_scenario_1_junior_uop_youth_relief_positive(self):
        ...
        response = self.app.post(
            "/api/calculate", data=json.dumps(request), content_type="application/json"
        )
    ...
    def test_scenario_2_senior_b2b_no_benefits_positive(self):
        request = self.base_request.copy()
        request["b2b"].update(
            {"monthly_invoice_amount": 25000, "zus_type": "full", "sickness_insurance": False}
        )
        response = self.app.post(
            "/api/calculate", data=json.dumps(request), content_type="application/json"
        )
        data = json.loads(response.data)["b2b_results"]
        # Full ZUS 2026 without sickness:
        # (1103.27+452.16+94.39+138.47)*12 = 1788.29*12 = 21459.48 + Zdrowotna
        ...
        request["b2b"].update(
            {
                "stoppage_months": 2,
                "vacation_days": 26,
                "companyBenefits": {"paidVacationDays": {"enabled": True, "value": 0, "days": 26}},
            }
        )
        response = self.app.post(
            "/api/calculate", data=json.dumps(request), content_type="application/json"
    ...
    def test_scenario_6_uop_full_benefits_positive(self):
        ...
        request["uop"].update(
            {
                "monthly_gross_salary": 15000,
                "selected_benefits": ["medical_care", "sport_card", "training_budget", "ppk"],
            }
        )
        response = self.app.post(
            "/api/calculate", data=json.dumps(request), content_type="application/json"
    ...
    def test_scenario_8_uop_realistic_leave_not_double_counted_positive(self):
        ...
        assert (
            data["total_annual_value"] == data["annual_net_income"] + data["annual_benefits_value"]
        )

    def test_scenario_9_b2b_ip_box_positive(self):
        request = self.base_request.copy()
        request["b2b"].update(
            {"monthly_invoice_amount": 20000, "tax_form": "ip_box", "vacation_days": 20}
        )
        response = self.app.post(
            "/api/calculate", data=json.dumps(request), content_type="application/json"
        )
    ...
    def test_scenario_16_b2b_negative_income_negative(self):
        request = self.base_request.copy()
        request["b2b"].update(
            {"monthly_invoice_amount": 4000, "stoppage_months": 4, "vacation_days": 30}
        )
        response = self.app.post(
            "/api/calculate", data=json.dumps(request), content_type="application/json"
        )
    ...
    def test_scenario_18_export_excel_positive(self):
        calc_response = self.app.post(
            "/api/calculate", data=json.dumps(self.base_request), content_type="application/json"
        )
        calc_data = json.loads(calc_response.data)
        response = self.app.post(
            "/api/export/excel", data=json.dumps(calc_data), content_type="application/json"
        )
    ...
    def test_scenario_19_b2b_tax_scale_positive(self):
        request = self.base_request.copy()
        request["b2b"].update(
            {"monthly_invoice_amount": 15000, "tax_form": "tax_scale", "zus_type": "full"}
        )
        response = self.app.post(
            "/api/calculate", data=json.dumps(request), content_type="application/json"
        )
