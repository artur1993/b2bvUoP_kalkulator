...
def compute_income_tax(
    ...
    if tax_form == "lump_sum_it":
        ...
        annual_tax = round_tax(round_tax(tax_base) * config["tax_thresholds"]["lump_sum_it"])
    ...
    taxable_base = income
    if tax_form == "flat_tax":
        annual_tax = round_tax(
            round_tax(max(0.0, taxable_base)) * config["tax_thresholds"]["flat_tax"]
        )
        ...
        qualified_tax = round_tax(round_tax(qualified_base) * config["tax_thresholds"]["ip_box"])
        other_tax = (
            progressive_tax(other_base, config)
            if base_form == "tax_scale"
            else round_tax(round_tax(other_base) * config["tax_thresholds"]["flat_tax"])
        )
