...
describe('calculatorReducer', () => {
  it('updates a flat B2B field', () => {
    const state = calculatorReducer(initialState, {
      type: 'SET_B2B_FIELD',
      payload: { name: 'monthly_invoice_amount', value: '15000', inputType: 'number', checked: false },
    });
  ...
  it('updates nested company benefits', () => {
    const state = calculatorReducer(initialState, {
      type: 'SET_B2B_FIELD',
      payload: { name: 'companyBenefits.paidVacationDays.enabled', value: 'on', inputType: 'checkbox', checked: true },
    });
  ...
  it('updates UoP selected benefits', () => {
    const state = calculatorReducer(initialState, {
      type: 'SET_UOP_FIELD',
      payload: { name: 'selected_benefits', value: 'ppk', inputType: 'checkbox', checked: true },
    });
  ...
  it('sets age in both data branches and toggles youth relief', () => {
    const state = calculatorReducer(initialState, { type: 'SET_AGE', payload: 24 });
  ...
  it('tracks calculation lifecycle', () => {
    const loadingState = calculatorReducer(initialState, { type: 'CALCULATE_START' });
    const successState = calculatorReducer(loadingState, { type: 'CALCULATE_SUCCESS', payload: { ok: true } });
