...
def calculate_function_coverage(coverage_data_path, source_dir):
    ...
    """
    try:
        with open(coverage_data_path, encoding="utf-8") as f:
    ...
    for file_path_relative, data in files_coverage.items():
        ...
        for _func_name, start_line, end_line in all_funcs_in_file:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            "Usage: python calculate_function_coverage.py <coverage_json_path> <source_directory>",
            file=sys.stderr,
        )
        sys.exit(1)
